...
TEST(touchmind_model_MapModel, test_001) {
  ...
  std::shared_ptr<MapModel> map = std::make_shared<MapModel>();
  map->SetSelectionManager(selectionManager.get());
  map->SetFileOperation(fileOperation.get());
  map->New();
  std::shared_ptr<NodeModel> node1 = NodeModel::Create(selectionManager.get());
  map->GetRootNodeModel()->AddChild(node1);
  std::shared_ptr<NodeModel> node2 = NodeModel::Create(selectionManager.get());
  map->GetRootNodeModel()->AddChild(node2);

  std::shared_ptr<LinkModel> link1 = LinkModel::Create(selectionManager.get());

...
void touchmind::selection::SelectionManager::NotifySelected(const std::shared_ptr<SelectableSupport> &selectedItem) {
  ...
  for (auto &l : m_selectionChangeListeners) {
    l(oldItem, newItem);
  }
}

void touchmind::selection::SelectionManager::NotifyUnselected(
    const std::shared_ptr<SelectableSupport> &unselectedItem) {
  if (m_selected == unselectedItem) {
    m_selected = nullptr;

    for (auto &l : m_selectionChangeListeners) {
      l(unselectedItem, nullptr);
    }
  }
}
...
std::shared_ptr<touchmind::ribbon::dispatch::INodeProperty>
touchmind::selection::SelectionManager::GetSelectedNodeProperty() const {
  return std::dynamic_pointer_cast<touchmind::ribbon::dispatch::INodeProperty>(GetSelected());
}

std::shared_ptr<touchmind::ribbon::dispatch::ILineProperty>
touchmind::selection::SelectionManager::GetSelectedLineProperty() const {

...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::model::node::NodeModel::Create(
    touchmind::selection::SelectionManager *pSelectionManager)
{
...
touchmind::model::node::NodeModel& touchmind::model::node::NodeModel::operator=(
    touchmind::model::node::NodeModel && other)
{
    if (this != &other) {
        m_id = other.m_id;
        m_position = other.m_position;
...
{
    if (std::find(std::begin(m_children), std::end(m_children), node) != std::end(m_children)) {
        return;
    }
...
void touchmind::model::node::NodeModel::RemoveChild(const std::shared_ptr<touchmind::model::node::NodeModel> &node)
{
    ...
    m_children.erase(std::remove(std::begin(m_children), std::end(m_children), node), std::end(m_children));
...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::model::node::NodeModel::_FindByNodeId(
...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::model::node::NodeModel::FindByNodeId(touchmind::NODE_ID nodeId)
{
...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::model::node::NodeModel::GetLeftSibling()
{
    if (GetParent() == nullptr) {
        ...
        for (auto it = std::begin(parent->m_children); it != std::end(parent->m_children); ++it) {
            if ((*it)->GetId() == GetId()) {
                if (it == std::begin(parent->m_children)) {
                    return nullptr;
                } else {
                    return *(it - 1);
                }
...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::model::node::NodeModel::GetRightSibling()
{
    if (GetParent() == nullptr) {
        ...
        for (auto it = std::begin(parent->m_children); it != std::end(parent->m_children); ++it) {
            if ((*it)->GetId() == GetId()) {
                if (it == (std::end(parent->m_children) - 1)) {
                    return nullptr;
                } else {
                    return *(it + 1);
                }
...
bool touchmind::model::node::NodeModel::IsAncestor(const std::shared_ptr<touchmind::model::node::NodeModel> &other) const
{
...
void _DeepCopy(
    std::shared_ptr<touchmind::model::node::NodeModel> srcParent,
...
bool _DeepCompare(
    const std::shared_ptr<touchmind::model::node::NodeModel> &node1,
...
void touchmind::model::node::NodeModel::_Dump(
    std::wostream& out,
    int indent,
    const std::shared_ptr<touchmind::model::node::NodeModel> &node)
{
...
{
    std::for_each(std::begin(m_links), std::end(m_links), [](std::weak_ptr<touchmind::model::link::LinkModel> _link) {
        if (!_link.expired()) {
...
std::wostream& touchmind::model::node::operator<<(std::wostream& os, touchmind::model::node::NodeModel const& node)
{
    ...
    os  << L"NodeModel"
        << L"[id=" << node.m_id
        << L",position=" << node.m_position
        << L",text=" << node.m_text
        << L",createdTime=" << createdTime
        << L",modifiedTime=" << modifiedTime
        << L",collapsed=" << node.m_collapsed
        << L",canCollapsed=" << node.m_canCollapsed
        << L",x=" << node.m_x
        << L",y=" << node.m_y
        << L",width=" << node.m_width
        << L",height=" << node.m_height
        << L",backgroundColor=" << node.m_backgroundColor;
    os << L",fontAttributes={";

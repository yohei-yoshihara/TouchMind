...
void touchmind::model::linkedge::LinkEdgeModel::CalculateAngleAndLength(D2D1_POINT_2F nodePos, D2D1_POINT_2F handlePos)
{
    D2D1_POINT_2F p = {
        std::abs(handlePos.x - nodePos.x),
        -(handlePos.y - nodePos.y)
    };
    m_angle = std::atan(p.y / p.x);
    m_length = std::sqrt(p.x * p.x + p.y * p.y);
}

D2D1_POINT_2F touchmind::model::linkedge::LinkEdgeModel::CalculateHandlePosition(
    D2D1_POINT_2F nodePos,
    touchmind::NODE_SIDE nodeSide) const
{
    LOG_ENTER_ARG(L"nodePos = " << nodePos << L", nodeSide = " << nodeSide);
    D2D1_POINT_2F p = {
        std::cos(m_angle) * m_length,
        -std::sin(m_angle) * m_length
    };
...
std::wostream& touchmind::model::linkedge::operator<<(std::wostream& os, const touchmind::model::linkedge::LinkEdgeModel &handle)
{
    os << L"LinkEdgeHandleModel[angle=" << handle.GetAngle()
       << L",length=" << handle.GetLength()
       << L"]";
    return os;
}

...
touchmind::model::link::LinkModel::Create(touchmind::selection::SelectionManager *pSelectionManager) {
...
void touchmind::model::link::LinkModel::SetNode(EDGE_ID edgeId,
...
D2D1_POINT_2F touchmind::model::link::LinkModel::GetEdgePoint(EDGE_ID edgeId) const {
  ...
  return D2D1::Point2F(node->GetAncestorPosition() == NODE_SIDE_RIGHT ? node->GetX() + node->GetWidth() + 2.0f
                                                                      : node->GetX() - 2.0f,

...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnLeftMouseDownExt(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
  ...
  util::CoordinateConversion::ConvertWindowToModelCoordinate(hWnd, m_pCanvasPanel->GetScrollBarHelper(), windowPoint,
                                                             &clientPoint);

  auto linkViewManager = m_pCanvasPanel->GetLinkViewManager();
  auto link = linkViewManager->HitTest(pContext, pRenderTarget, clientPoint);
  if (link != nullptr) {
    link->SetSelected();
  } else {
    ...
    if (path != nullptr) {
      ...
      auto node = nodeViewManager->NodeHitTest(m_pCanvasPanel->GetContext(),
                                               m_pCanvasPanel->GetContext()->GetD2DRenderTarget(), clientPoint);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnSingleClick(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                    ULONGLONG timestampInMilliSeconds) {
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnDoubleClick(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                    ULONGLONG timestampInMilliSeconds) {
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
  UNREFERENCED_PARAMETER(timestampInMilliSeconds);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnDraggingStart(HWND hWnd, WPARAM wParam, LPARAM lParam, RECT windowDragRect,
  ...
  util::CoordinateConversion::ConvertWindowToModelCoordinate(
      hWnd, m_pCanvasPanel->GetScrollBarHelper(), windowDragRect.left, windowDragRect.top, &startPointInModel);
  ...
  if (pHitNode != nullptr) {
    ...
    if (pHitNode->GetWidth() - 10.0f <= xInNode && xInNode <= pHitNode->GetWidth()
      ...
      m_rectBeforeResize = D2D1::RectF(pHitNode->GetX(), pHitNode->GetY(), pHitNode->GetX() + pHitNode->GetWidth(),
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnDragging(HWND hWnd, WPARAM wParam, LPARAM lParam, RECT windowDragRect, LONG dx,
  ...
  case MANIPULATION_STATUS_RESIZING_NODE: {
    if (m_pResizingNode != nullptr) {
      ...
      util::CoordinateConversion::ConvertWindowToModelCoordinate(hWnd, m_pCanvasPanel->GetScrollBarHelper(),
                                                                 windowDragRect, &rect);
      m_rectOnResize = D2D1::RectF(m_pResizingNode->GetX(), m_pResizingNode->GetY(), rect.right, rect.bottom);
      m_pCanvasPanel->ResizeLayoutRect(m_pResizingNode, m_rectOnResize.right - m_rectOnResize.left,
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnDraggingEnd(HWND hWnd, WPARAM wParam, LPARAM lParam, RECT windowDragRect,
                                                    LONG dx, LONG dy, ULONGLONG timestampInMilliSeconds) {
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
  UNREFERENCED_PARAMETER(timestampInMilliSeconds);
  ...
  case MANIPULATION_STATUS_MOVING_NODE: {
    if (m_pMovingNode != nullptr) {
      D2D1_POINT_2F clientPoint;
      util::CoordinateConversion::ConvertWindowToModelCoordinate(
          hWnd, m_pCanvasPanel->GetScrollBarHelper(), windowDragRect.right, windowDragRect.bottom, &clientPoint);
      touchmind::visitor::NodeHitTestVisitor hitTestVisitor(clientPoint, GetHitTestMargin());
      m_pCanvasPanel->GetRootNodeModel()->ApplyVisitor(hitTestVisitor);
      std::shared_ptr<touchmind::model::node::NodeModel> pHitNode = hitTestVisitor.GetResult();
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnHovering(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint, LONG dx,
                                                 LONG dy, ULONGLONG timestampInMilliSeconds) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnRightSingleClick(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                         ULONGLONG timestampInMilliSeconds) {
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
  UNREFERENCED_PARAMETER(timestampInMilliSeconds);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnGestureBegin(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnGestureEnd(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                   ULONGLONG timestampInMilliSeconds, PGESTUREINFO pGestureInfo) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnZoom(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                             ULONGLONG timestampInMilliSeconds, PGESTUREINFO pGestureInfo,
  ...
  if (pGestureInfo->dwFlags == GF_BEGIN) {
    ...
    m_pResizingNode = std::dynamic_pointer_cast<touchmind::model::node::NodeModel>(
        m_pCanvasPanel->GetSelectionManager()->GetSelected());
    if (m_pResizingNode != nullptr) {
      m_manipulationStatus = MANIPULATION_STATUS_RESIZING_NODE_WITH_ZOOM_GESTURE;
      m_rectBeforeResize = m_rectOnResize = D2D1::RectF(m_pResizingNode->GetX(), m_pResizingNode->GetY(),
    ...
    SetMouseCursor(MOUSE_CURSOR_TYPE_DEFAULT);
  } else if (std::dynamic_pointer_cast<touchmind::model::node::NodeModel>(
                 m_pCanvasPanel->GetSelectionManager()->GetSelected())
                 != nullptr
             && zoomFactor > 0.1f) {
    m_manipulationStatus = MANIPULATION_STATUS_RESIZING_NODE_WITH_ZOOM_GESTURE;
    ...
    m_rectOnResize = D2D1::RectF(m_pResizingNode->GetX(), m_pResizingNode->GetY(), m_pResizingNode->GetX() + newWidth,
...
touchmind::touch::ManipulationResult touchmind::touch::ManipulationHelper::OnPan(HWND hWnd, WPARAM wParam,
                                                                                 LPARAM lParam, POINT windowPoint,
                                                                                 ULONGLONG timestampInMilliSeconds,
                                                                                 PGESTUREINFO pGestureInfo, LONG dx,
                                                                                 LONG dy) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
  ...
  if (pGestureInfo->dwFlags & GF_BEGIN) {
    D2D1_POINT_2F startPointInModel;
    util::CoordinateConversion::ConvertWindowToModelCoordinate(hWnd, m_pCanvasPanel->GetScrollBarHelper(),
                                                               windowPoint.x, windowPoint.y, &startPointInModel);
    ...
    if (pGestureInfo->dwFlags & GF_END) {
      if (m_pMovingNode != nullptr) {
        ...
        D2D1_POINT_2F clientPoint;
        util::CoordinateConversion::ConvertWindowToModelCoordinate(hWnd, m_pCanvasPanel->GetScrollBarHelper(),
                                                                   windowPoint.x, windowPoint.y, &clientPoint);
        touchmind::visitor::NodeHitTestVisitor hitTestVisitor(clientPoint, GetHitTestMargin());
        m_pCanvasPanel->GetRootNodeModel()->ApplyVisitor(hitTestVisitor);
        std::shared_ptr<touchmind::model::node::NodeModel> pHitNode = hitTestVisitor.GetResult();
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnRotate(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                               ULONGLONG timestampInMilliSeconds, PGESTUREINFO pGestureInfo,
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnTwoFingerTap(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                     ULONGLONG timestampInMilliSeconds, PGESTUREINFO pGestureInfo) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);
...
touchmind::touch::ManipulationResult
touchmind::touch::ManipulationHelper::OnPressAndTap(HWND hWnd, WPARAM wParam, LPARAM lParam, POINT windowPoint,
                                                    ULONGLONG timestampInMilliSeconds, PGESTUREINFO pGestureInfo) {
  UNREFERENCED_PARAMETER(hWnd);
  UNREFERENCED_PARAMETER(wParam);
  UNREFERENCED_PARAMETER(lParam);

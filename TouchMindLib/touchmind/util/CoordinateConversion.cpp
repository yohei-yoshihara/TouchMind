...
void touchmind::util::CoordinateConversion::ConvertScreenToModelCoordinate(
    HWND hWnd, touchmind::util::ScrollBarHelper *pScrollBarHelper, POINT point, D2D1_POINT_2F *clientPoint) {
  RECT clientRect;
...
void touchmind::util::CoordinateConversion::ConvertScreenToModelCoordinate(
    HWND hWnd, touchmind::util::ScrollBarHelper *pScrollBarHelper, LONG x, LONG y, D2D1_POINT_2F *modelPoint) {
  POINT p;
  p.x = x;
  p.y = y;
...
void touchmind::util::CoordinateConversion::ConvertWindowToModelCoordinate(
    HWND hWnd, touchmind::util::ScrollBarHelper *pScrollBarHelper, RECT rect, D2D1_RECT_F *clientRect) {
  POINT windowPoint;
...
void touchmind::util::CoordinateConversion::ConvertModelToWindowCoordinate(
    HWND hWnd, touchmind::util::ScrollBarHelper *pScrollBarHelper, D2D1_POINT_2F clientPoint, POINT *point) {
  RECT clientRect;
  ::GetClientRect(hWnd, &clientRect);
  INT clientWidth = clientRect.right - clientRect.left + 1;
  INT clientHeight = clientRect.bottom - clientRect.top + 1;

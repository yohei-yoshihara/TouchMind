...
namespace touchmind {
  ...
  enum VISITOR_RESULT { VISITOR_RESULT_CONTINUE, VISITOR_RESULT_STOP };

  enum NODE_SIDE { NODE_SIDE_RIGHT = 0, NODE_SIDE_LEFT = 1, NODE_SIDE_UNDEFINED = -1 };
  std::wostream &operator<<(std::wostream &os, const NODE_SIDE &nodeSide);

  enum PATH_DIRECTION { PATH_DIRECTION_RIGHT, PATH_DIRECTION_LEFT, PATH_DIRECTION_AUTO };
...
#define _TEMPFILE_PREFIX L"TouchMind_"
  ...
  extern size_t LengthInUTF8(IN const wchar_t *utf16);
  extern int UTF16ToUTF8(IN const wchar_t *utf16, OUT char *utf8, IN size_t n);
  extern size_t LengthInUTF16(IN const char *utf8);
  extern int UTF8ToUTF16(IN const char *utf8, OUT wchar_t *utf16, IN size_t n);

...
errno_t touchmind::SystemtimeToString(IN const SYSTEMTIME *pSystemTime, OUT std::wstring &s) {
  wchar_t buf[1024];
  swprintf_s(buf, L"%04u%02u%02u%02u%02u%02u%03u", pSystemTime->wYear, pSystemTime->wMonth, pSystemTime->wDay,
             pSystemTime->wHour, pSystemTime->wMinute, pSystemTime->wSecond, pSystemTime->wMilliseconds);
  s = buf;
  return 0;
}
...
errno_t touchmind::StringToSystemtime(IN std::wstring &s, OUT SYSTEMTIME *pSystemTime) {
  ...
  swscanf_s(s.c_str(), L"%4u%2u%2u%2u%2u%2u%3u", &year, &month, &day, &hour, &minute, &second, &milliseconds);
...
size_t touchmind::LengthInUTF8(const wchar_t *utf16) {
  return ::WideCharToMultiByte(CP_UTF8, 0, utf16, -1, nullptr, 0, nullptr, nullptr);
}

int touchmind::UTF16ToUTF8(const wchar_t *utf16, char *utf8, size_t n) {
  if (n < 1) {
    // too short
    return 0;
  }
  size_t utf16Len = wcslen(utf16);
  size_t utf8Len = LengthInUTF8(utf16);
  if (n < utf8Len) {
    return 0;
  }
  int ret = ::WideCharToMultiByte(CP_UTF8, 0, utf16, static_cast<int>(utf16Len), utf8, static_cast<int>(n), nullptr,
                                  nullptr);
  if (ret != 0) {
    utf8[ret] = '\0';
  }
...
size_t touchmind::LengthInUTF16(const char *utf8) {
  return ::MultiByteToWideChar(CP_UTF8, 0, utf8, -1, nullptr, 0);
}

int touchmind::UTF8ToUTF16(const char *utf8, wchar_t *utf16, size_t n) {
  if (n < 1) {
    // too short
    return 0;
  }
  size_t utf8Len = strlen(utf8);
  size_t utf16Len = LengthInUTF16(utf8);
  if (n < utf16Len) {
    utf16[0] = '\0';
    return 0;
  }
  int ret = ::MultiByteToWideChar(CP_UTF8, 0, utf8, static_cast<int>(utf8Len), utf16, static_cast<int>(n));
  if (ret != 0) {
    utf16[ret] = '\0';
  }
  return ret;

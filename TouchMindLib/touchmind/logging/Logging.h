...
{                                                                                                                 \
    HRESULT __hr__ = expression;                                                                                       \
    LOG(SEVERITY_LEVEL_DEBUG) << L"[GPU RESOURCE] " << #resource << L"[" << typeid((resource)).name() << L"] = ["      \
                              << std::hex << (resource) << std::dec << L"]";                                           \
    if (FAILED(__hr__)) {                                                                                              \
      LOG(SEVERITY_LEVEL_FATAL) << #expression << L", hr = " << std::hex << __hr__ << std::dec;                        \
      throw std::runtime_error(#expression);                                                                           \
    }                                                                                                                  \
...
#define LOG_RES(resource)                                                                                              \
  LOG(SEVERITY_LEVEL_DEBUG) << L"[RESOURCE] " << #resource << L"[" << typeid((resource)).name() << L"] = ["            \
                            << std::hex << (resource) << std::dec << L"]"
#else
#define LOG_RES(resource)
#endif
...
#define LOG_ENTER_ARG(args)                                                                                            \
  BOOST_LOG_SEV(logger::get(), SEVERITY_LEVEL_DEBUG) << __FUNCTION__ << L"(" << __LINE__ << L") <ENTER> - " << args

#define LOG_LEAVE BOOST_LOG_SEV(logger::get(), SEVERITY_LEVEL_DEBUG) << __FUNCTION__ << L"(" << __LINE__ << L") <LEAVE>"
#define LOG_LEAVE_ARG(args)                                                                                            \
  BOOST_LOG_SEV(logger::get(), SEVERITY_LEVEL_DEBUG) << __FUNCTION__ << L"(" << __LINE__ << L") <LEAVE> - " << args
#define LOG_LEAVE_HRESULT(hr)                                                                                          \
  BOOST_LOG_SEV(logger::get(), SEVERITY_LEVEL_DEBUG) << __FUNCTION__ << L"(" << __LINE__ << L") <LEAVE> - "            \
                                                     << L"HRESULT = " << touchmind::logging::HResultStr(hr)

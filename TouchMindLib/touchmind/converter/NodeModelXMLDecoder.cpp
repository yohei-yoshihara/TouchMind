...
HRESULT touchmind::converter::NodeModelXMLDecoder::Decode(
    IN MSXML::IXMLDOMNodePtr xmlNodeNode,
    ...
    if (s_node == xmlNodeNode->baseName) {
        ...
        if (pIdNode != nullptr) {
            ...
            if (touchmind::StringToNodeId(ws_id, &nodeId) == 0) {
                if (keepOriginalId) {
                    idMap->insert(std::make_pair<NODE_ID, NODE_ID>(nodeId, nodeId));
                    node->SetId(nodeId);
                } else {
                    idMap->insert(std::make_pair<NODE_ID, NODE_ID>(nodeId, node->GetId()));
                }
            }
        }
        ...
        for (int i = 0; i < pNodeList->length; ++i) {
            ...
            if (pChildNode->baseName == s_text) {
                ...
                // child node
                auto pChildNodeModel =
                    touchmind::model::node::NodeModel::Create(m_pSelectionManager);
...
HRESULT touchmind::converter::NodeModelXMLDecoder::_DecodeLink(
    ...
    std::shared_ptr<LinkInfoList> linkInfoList)
{
    ...
    linkInfo.link =
        touchmind::model::link::LinkModel::Create(m_pSelectionManager);
...
HRESULT touchmind::converter::NodeModelXMLDecoder::_DecodePath(
...
HRESULT touchmind::converter::NodeModelXMLDecoder::_DecodeText(

...
namespace touchmind {
  namespace view {
    namespace linkedge {
      ...
      class BaseLinkEdgeView : public touchmind::view::BaseView {
      private:
        std::weak_ptr<touchmind::model::link::LinkModel> m_link;
        EDGE_ID m_edgeId;

      public:
        BaseLinkEdgeView(std::weak_ptr<touchmind::model::link::LinkModel> link, EDGE_ID edgeId);
        ...
        virtual void Draw(touchmind::Context *pContext, ID2D1RenderTarget *pRenderTarget, ID2D1Brush *pBrush,
                          FLOAT width = 1.0f)
            = 0;
        virtual void Fill(touchmind::Context *pContext, ID2D1RenderTarget *pRenderTarget, ID2D1Brush *pBrush = nullptr)
            = 0;

...
/* static */
void touchmind::view::node::NodeViewManager::CreateBodyColor(
    IN COLORREF baseColor,
    OUT D2D1_COLOR_F &startColor,
...
void touchmind::view::node::NodeViewManager::DrawNodeShadow(
...
void touchmind::view::node::NodeViewManager::DrawSelectedNodeShadow(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget,
    ID2D1Geometry *pGeometry)
{
...
public:
    SynchronizeViewBeforeArrangingVisitor(
    ...
    void CreateNewView( std::shared_ptr<touchmind::model::node::NodeModel> node, touchmind::control::EDIT_CONTROL_INDEX editControlIndex ) {
        auto nodeView = touchmind::view::node::NodeViewFactory::Create(node);
        nodeView->SetNodeViewManager(m_nodeViewManager);
        nodeView->SetEditControlManager(m_pEditControlManager);
        nodeView->SetEditControlIndex(editControlIndex);
...
public:
    ...
    touchmind::VISITOR_RESULT operator()(std::shared_ptr<touchmind::model::node::NodeModel> node) {
        D2D1_RECT_F layoutRect = D2D1::RectF(
                                     node->GetX() + m_insets.left,
                                     node->GetY() + m_insets.top,
        ...
        if (m_nodeIdToView.count(node->GetId()) != 0) {
            ...
            std::shared_ptr<touchmind::control::DWriteEditControl> editControl = m_pEditControlManager->GetEditControl(editControlIndex);
...
HRESULT touchmind::view::node::NodeViewManager::CreateSharedDeviceResources(
    touchmind::Context *pContext,
    ID2D1RenderTarget* pRenderTarget)
{
    UNREFERENCED_PARAMETER(pContext);
    if (m_pNodeShadowBrush == nullptr ||
            m_pSelectedNodeShadowBrush1 == nullptr ||
            m_pSelectedNodeShadowBrush2 == nullptr) {
        m_pNodeShadowBrush = nullptr;
        CHK_RES(m_pNodeShadowBrush, pRenderTarget->CreateSolidColorBrush(
                    m_pConfiguration->GetDefaultShadowColor(),
                    &m_pNodeShadowBrush));

        m_pSelectedNodeShadowBrush1 = nullptr;
        CHK_RES(m_pSelectedNodeShadowBrush1, pRenderTarget->CreateSolidColorBrush(
                    m_pConfiguration->GetDefaultSelectedColor1(),
                    &m_pSelectedNodeShadowBrush1));

        m_pSelectedNodeShadowBrush2 = nullptr;
        CHK_RES(m_pSelectedNodeShadowBrush2, pRenderTarget->CreateSolidColorBrush(
...
HRESULT touchmind::view::node::NodeViewManager::DrawNode(
    touchmind::Context *pContext,
    ID2D1RenderTarget* pRenderTarget,
...
void touchmind::view::node::NodeViewManager::DrawPath(
    touchmind::Context *pContext,
    ID2D1RenderTarget* pRenderTarget,
...
HRESULT touchmind::view::node::NodeViewManager::SynchronizeWithModel()
{
    ...
    for (auto it = std::begin(m_nodeIdToViewMap); it != std::end(m_nodeIdToViewMap); ++it) {
        auto nodeView = it->second;
        nodeView->ClearHandled();
    }
    // synchronize view resource with model
    SynchronizeViewAfterArrangingVisitor visitor(this, m_nodeIdToViewMap, m_pEditControlManager, m_pConfiguration->GetInsets());
    ...
    for (auto it = std::begin(m_nodeIdToViewMap); it != std::end(m_nodeIdToViewMap); ++it) {
        ...
        if (!nodeView->IsHandled()) {
            ...
            prepareRemove.push_back(it->first);
        }
    }
    // remove view resources
    for (auto it = std::begin(prepareRemove); it != std::end(prepareRemove); ++it) {
        touchmind::NODE_ID nodeId = *it;
...
touchmind::NODE_ID touchmind::view::node::NodeViewManager::GetNodeIdFromEditControlIndex(control::EDIT_CONTROL_INDEX editControlIndex)
{
    for (auto it = std::begin(m_nodeIdToViewMap); it != std::end(m_nodeIdToViewMap); ++it) {
        touchmind::NODE_ID nodeId = (*it).first;
        auto nodeView = (*it).second;
...
HRESULT touchmind::view::node::NodeViewManager::_CreateEditControl(
...
HRESULT touchmind::view::node::NodeViewManager::SyncFontAttributesToTextLayout()
{
    ...
    for (auto it = std::begin(m_nodeIdToViewMap); it != std::end(m_nodeIdToViewMap); ++it) {
        ...
        std::shared_ptr<control::DWriteEditControl> pEditControl = m_pEditControlManager->GetEditControl(nodeView->GetEditControlIndex());
...
void touchmind::view::node::NodeViewManager::DrawNodes(
    touchmind::Context *pContext,
    ID2D1RenderTarget* pRenderTarget,
...
void touchmind::view::node::NodeViewManager::DrawNodePaths(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget,
    std::shared_ptr<model::node::NodeModel> node)
{
...
void touchmind::view::node::NodeViewManager::OnDebugDump(std::wofstream& os)
{
    ...
    for (auto it = std::begin(m_nodeIdToViewMap); it != std::end(m_nodeIdToViewMap); ++it) {
        touchmind::NODE_ID nodeId = it->first;
        auto &nodeView = it->second;
        os << L"nodeId = " << nodeId << L" -> NodeView"
           << L"[editControlIndex=" << nodeView->GetEditControlIndex()
           << L",isHandled=" << std::boolalpha << nodeView->IsHandled()
           << std::endl;
    }
...
std::shared_ptr<touchmind::model::node::NodeModel> touchmind::view::node::NodeViewManager::NodeHitTest(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget,
    D2D1_POINT_2F point)
{
...
std::shared_ptr<touchmind::model::path::PathModel> touchmind::view::node::NodeViewManager::PathHitTest(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget,
    D2D1_POINT_2F point)
{

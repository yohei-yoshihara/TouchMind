...
touchmind::view::node::impl::RoundedRectNodeView::RoundedRectNodeView(
    std::weak_ptr<touchmind::model::node::NodeModel> node)
    : BaseNodeView(node)
    , m_pTexture(nullptr) {
}
...
void touchmind::view::node::impl::RoundedRectNodeView::CreateDeviceDependentResources(
...
void touchmind::view::node::impl::RoundedRectNodeView::Draw(touchmind::Context *pContext,
  ...
  if (m_pTexture == nullptr || node->IsRepaintRequired(GetRepaintCounter())) {
    ...
    D2D1_RECT_F bodyRect
        = D2D1::RectF(shadowOffset + d, shadowOffset + d, shadowOffset + node->GetWidth() + bodyOffset * 2 + d,
                      shadowOffset + node->GetHeight() + bodyOffset * 2 + d);

    UINT nWidth = static_cast<UINT>(node->GetWidth() + 0.5f + (bodyOffset + shadowOffset) * 2.0f);
    UINT nHeight = static_cast<UINT>(node->GetHeight() + 0.5f + (bodyOffset + shadowOffset) * 2.0f);
    ...
    GetNodeViewManager()->CreateBodyColor(util::ColorUtil::ToColorref(node->GetBackgroundColor()), startColor,
                                          endColor);
    nodeFigure.SetPlateStartColor(startColor);
    nodeFigure.SetPlateEndColor(endColor);
    nodeFigure.DrawPlate(bodyRect, nodeFigure.GetCornerRoundSize(), pContext->GetD2DFactory(),
                         pOutputTexture2DRenderTarget);
...
bool touchmind::view::node::impl::RoundedRectNodeView::IsCompatible(
  ...
  if (node != nullptr) {
    return node->GetNodeShape() == NODE_SHAPE_ROUNDED_RECTANGLE;
  }
  return false;
}

void touchmind::view::node::impl::RoundedRectNodeView::DiscardDeviceDependentResources() {

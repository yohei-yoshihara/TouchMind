...
void touchmind::view::path::impl::DefaultPathView::CreateDeviceDependentResources(touchmind::Context *pContext,
  ...
  if (path->IsRepaintRequired(GetRepaintCounter()) || m_pathBrush == nullptr || m_pathGeometry == nullptr
      || m_pathStyle == nullptr) {
...
void touchmind::view::path::impl::DefaultPathView::Draw(touchmind::Context *pContext,
                                                        ID2D1RenderTarget *pRenderTarget) {
  if (!_CheckValidity()) {
    return;
  }
  ...
  pRenderTarget->DrawGeometry(m_pathGeometry, m_pathBrush, path->GetWidthAsValue(), m_pathStyle);
}

bool touchmind::view::path::impl::DefaultPathView::HitTest(touchmind::Context *pContext,
...
void touchmind::view::path::impl::DefaultPathView::_CreateTexture(
  ...
  pSourceTexture2DRenderTarget->SetTransform(
      D2D1::Matrix3x2F::Translation(-m_bounds.left + shadowOffset, -m_bounds.top + shadowOffset));
  pSourceTexture2DRenderTarget->DrawGeometry(m_pathGeometry, GetNodeViewManager()->GetSelectedShadowBrush1(),

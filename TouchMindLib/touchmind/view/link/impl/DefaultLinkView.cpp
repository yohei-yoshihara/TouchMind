...
void touchmind::view::link::impl::DefaultLinkView::CreateDeviceDependentResources(
    ...
    ID2D1RenderTarget *pRenderTarget)
{
    ...
    if (link->IsRepaintRequired(GetRepaintCounter()) ||
            m_linkBrush == nullptr ||
...
void touchmind::view::link::impl::DefaultLinkView::Draw(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget)
{
    if (HasLinkModel()) {
...
bool touchmind::view::link::impl::DefaultLinkView::HitTest(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget,
    D2D1_POINT_2F point)
{
    ...
    if (HasLinkModel()) {
        CreateDeviceDependentResources(pContext, pRenderTarget);
        BOOL b;
        m_linkGeometry->StrokeContainsPoint(
            point,
            GetLinkModel().lock()->GetLineWidth() + 5.0f,
            nullptr,
            nullptr,
            &b);
        result = (b == TRUE);
    ...
    std::shared_ptr<touchmind::model::link::LinkModel> link)
{
    ...
    m_linkGeometry = nullptr;
    CHK_RES(m_linkGeometry,
            GeometryBuilder::CreateCurvePathGeometry(pD2DFactory, curvePoints, &m_linkGeometry));
    ...
    std::shared_ptr<touchmind::model::link::LinkModel> link)
{
    ...
    pSourceTexture2DRenderTarget->SetTransform(D2D1::Matrix3x2F::Translation(-bounds.left + shadowOffset, -bounds.top + shadowOffset));
    pSourceTexture2DRenderTarget->DrawGeometry(m_linkGeometry, GetLinkViewManager()->GetSelectedShadowBrush1(), link->GetLineWidthAsValue() + 2.0f);
    pSourceTexture2DRenderTarget->DrawGeometry(m_linkGeometry, GetLinkViewManager()->GetSelectedShadowBrush2(), link->GetLineWidthAsValue());
    ...
    CHK_RES(pTexture2D,
            GetLinkViewManager()->GetGaussFilter()->ApplyFilter(pContext, pSourceTexutre2D, &pTexture2D));
    m_pBitmap = nullptr;
    CHK_RES(m_pBitmap,

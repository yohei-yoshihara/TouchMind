...
STDAPI touchmind::control::DWriteEditControlTextEditSink::OnEndEdit(ITfContext *pic, TfEditCookie ecReadOnly,
  ...
  if (SUCCEEDED(hr)) {
    IEnumTfRanges *pEnum;
    hr = pEditRecord->GetTextAndPropertyUpdates(TF_GTP_INCL_TEXT, dispAttrProps.GetPropTablePointer(),
                                                static_cast<ULONG>(dispAttrProps.Count()), &pEnum);
    if (SUCCEEDED(hr) && pEnum) {
      ...
      if (pEnum->Next(1, &pRange, nullptr) == S_OK) {
        ...
        if (SUCCEEDED(pic->GetStart(ecReadOnly, &pRangeEntire)) && SUCCEEDED(pic->GetEnd(ecReadOnly, &pRangeEnd))
...
void touchmind::control::DWriteEditControlTextEditSink::SetDisplayAttributeMgr(
    ITfDisplayAttributeMgr *pDisplayAttributeMgr) {

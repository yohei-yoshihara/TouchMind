...
namespace touchmind {
  namespace control {
    ...
    protected:
      ...
      void UpdateCaret();
      void DestroyCaret();
      void _GetFontAttributes(IN IDWriteTextLayout *pTextLayout, IN DWRITE_TEXT_RANGE &textRange,
                              OUT touchmind::text::FontAttributeCommand *pFontInfo,
                              OUT DWRITE_TEXT_RANGE *pActualTextRange = nullptr);
      void _GetFontAttributes(IN IDWriteTextLayout *pTextLayout, IN LONG acpStart,
                              OUT touchmind::text::FontAttributeCommand *pFontInfo);
      void _SetFontAttributes(IN IDWriteTextLayout *pTextLayout, IN touchmind::text::FontAttributeCommand *pFontInfo,
    ...
    public:
      DWriteEditControl(DWriteEditControlManager *manager, D2D1_RECT_F layoutRect,
      ...
      void GetLineFromPosition(const DWRITE_LINE_METRICS *lineMetrics, UINT32 lineCount, UINT32 textPosition,
                               OUT UINT32 *lineOut, OUT UINT32 *linePositionOut);
      bool CalculateNewCaretPositionForUpDown(DWORD cursorDirection, OUT LONG *caretPosition,
                                              OUT TsActiveSelEnd *activeSelEnd);
      bool CalculateSelectionFromDragRect(D2D1_POINT_2F startPoint, D2D1_POINT_2F endPoint, OUT LONG *acpStart,
                                          OUT LONG *acpEnd, OUT TsActiveSelEnd *activeSelEnd);
      void Invalidate();
      ...
      HRESULT SetFontAttributes(touchmind::Configuration *pConfiguration,
                                std::vector<touchmind::text::FontAttribute> &fontAttributes);

      void NotifySelectionHasChanged();

      // for Font Properties
      void CopySinglePropertyRange(IDWriteTextLayout *oldLayout, UINT32 startPosForOld, IDWriteTextLayout *newLayout,
                                   UINT32 startPosForNew, UINT32 length,
                                   touchmind::text::FontAttributeCommand *pFontAttributeCommand = nullptr);
      void CopyRangedProperties(IDWriteTextLayout *oldLayout, UINT32 startPos, UINT32 endPos,

...
HRESULT touchmind::control::DWriteEditControlDisplayAttribute::GetDisplayAttributeProperties(
    OUT DisplayAttributeProperties &displayAttributeProperties) {
...
HRESULT touchmind::control::DWriteEditControlDisplayAttribute::GetDisplayAttributeTrackPropertyRange(
    IN ITfContext *pTfContext, IN DisplayAttributeProperties &displayAttributeProperties,
...
HRESULT touchmind::control::DWriteEditControlDisplayAttribute::GetDisplayAttributeData(
    IN TfEditCookie editCookie, IN ITfReadOnlyProperty *pProp, IN ITfRange *pRange,
  ...
  if (SUCCEEDED(hr)) {
    hr = var.punkVal->QueryInterface(IID_IEnumTfPropertyValue, (void **)&pEnumPropertyVal);
    if (SUCCEEDED(hr)) {
...
HRESULT touchmind::control::DWriteEditControlDisplayAttribute::GetAttributeColor(IN const TF_DA_COLOR &daColor,
                                                                                 OUT COLORREF &colorref) {
  switch (daColor.type) {
  case TF_CT_SYSCOLOR:

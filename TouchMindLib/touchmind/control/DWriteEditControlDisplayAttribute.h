...
public:
    ...
    bool FindGuid(REFGUID rguid) const {
        return std::any_of(std::begin(m_guid), std::end(m_guid), [&](REFGUID e) {
            return IsEqualGUID(e, rguid);
        });
    }
...
public:
    ...
    HRESULT GetDisplayAttributeTrackPropertyRange(IN ITfContext *tfContext, IN DisplayAttributeProperties& displayAttributeProperties, OUT ITfReadOnlyProperty **ppProp);
    HRESULT GetDisplayAttributeData(IN TfEditCookie editCookie, IN ITfReadOnlyProperty *pProp, IN ITfRange *pRange, OUT TF_DISPLAYATTRIBUTE* pDisplayAttribute, OUT TfGuidAtom* pGuid);

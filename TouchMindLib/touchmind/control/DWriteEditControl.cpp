...
touchmind::control::DWriteEditControl::DWriteEditControl(DWriteEditControlManager *manager, D2D1_RECT_F layoutRect,
...
HRESULT touchmind::control::DWriteEditControl::InitializeTextFormat() {
  ...
  HRESULT hr = m_pManager->GetDWriteFactory()->CreateTextFormat(
      m_caretFontAttributeCommand.fontFamilyName.c_str(), nullptr, m_caretFontAttributeCommand.GetFontWeight(),
      m_caretFontAttributeCommand.GetFontStyle(), m_caretFontAttributeCommand.GetFontStretch(),
      m_caretFontAttributeCommand.fontSize, m_caretFontAttributeCommand.localeName.c_str(), &m_pDWriteTextFormat);
#ifdef DEBUG_GPU_RESOURCE
  LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] IDWriteTextFormat = [" << std::hex << m_pDWriteTextFormat << L"]"
                           << std::dec;
#endif
  if (FAILED(hr)) {
...
void touchmind::control::DWriteEditControl::_RecreateLayout() {
  ...
  if (m_pDWriteTextLayout == nullptr) {
    hr = m_pManager->GetDWriteFactory()->CreateTextLayout(text.c_str(), static_cast<UINT32>(text.length()),
                                                          m_pDWriteTextFormat, width, height, &m_pDWriteTextLayout);
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] IDWriteTextLayout = [" << std::hex << m_pDWriteTextLayout << L"]"
                             << std::dec;
#endif
    if (FAILED(hr)) {
    ...
    hr = m_pManager->GetDWriteFactory()->CreateTextLayout(text.c_str(), static_cast<UINT32>(text.length()),
                                                          oldTextLayout, width, height, &m_pDWriteTextLayout);
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] IDWriteTextLayout = [" << std::hex << m_pDWriteTextLayout << L"]"
                             << std::dec;
#endif
...
void touchmind::control::DWriteEditControl::_GetFontAttributes(IN IDWriteTextLayout *pTextLayout, IN LONG startPosition,
...
/// if the length of the textRange is zero, try to get font attributes at the startPosition of the textRange. This case,
/// any attributes of the FontAttributeCommand can not be UI_FONTPROPERTIES_NOTAVAILABLE
/// if the length of the textRange is more than zero, try to get font attributes at the text range. This case, some
/// attributes of FontAttributeCommand may be UI_FONTPROPERTIES_NOTAVAILABLE
void touchmind::control::DWriteEditControl::_GetFontAttributes(IN IDWriteTextLayout *pTextLayout,
                                                               IN DWRITE_TEXT_RANGE &textRange,
                                                               OUT touchmind::text::FontAttributeCommand *pFontInfo,
                                                               OUT DWRITE_TEXT_RANGE *pActualTextRange) {
  HRESULT hr;
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = pTextLayout->GetFontFamilyName(textRange.startPosition, fontFamilyName.data(), fontFamilyNameLength + 1,
                                        &actualTextRange);
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (endPos > actualEndPos) {
      ...
      pFontInfo->bold = fontWeight != DWRITE_FONT_WEIGHT_NORMAL ? UI_FONTPROPERTIES_SET : UI_FONTPROPERTIES_NOTSET;
    }
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (endPos > actualEndPos) {
      ...
      pFontInfo->italic = fontStyle != DWRITE_FONT_STYLE_NORMAL ? UI_FONTPROPERTIES_SET : UI_FONTPROPERTIES_NOTSET;
    }
  }
...
void touchmind::control::DWriteEditControl::_SetFontAttributes(
    IDWriteTextLayout *pTextLayout, IN touchmind::text::FontAttributeCommand *pFontAttributeCommand,
    IN DWRITE_TEXT_RANGE &textRange) {
  HRESULT hr;
...
void touchmind::control::DWriteEditControl::RenderTextWithCompositionUnderline(ID2D1RenderTarget *pRenderTarget,
  ...
  hr = m_pManager->GetDWriteFactory()->CreateTextLayout(text.c_str(), static_cast<UINT32>(text.length()),
                                                        m_pDWriteTextLayout, width, height, &pTextLayout);
#ifdef DEBUG_GPU_RESOURCE
  ...
  for (auto &ca : m_compositionAttributes) {
    if (ca.startPos >= 0 && ca.endPos >= 0 && ca.startPos != ca.endPos) {
      UINT32 actualHitTextCount;
      hr = m_pDWriteTextLayout->HitTestTextRange(ca.startPos, ca.endPos - ca.startPos, x, y, nullptr, 0,
                                                 &actualHitTextCount);
...
#ifdef DEBUG_GPU_RESOURCE
            LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << pLineColorBrush
                                     << L"]" << std::dec;
#endif
            if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
            LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << pTextColorBrush
                                     << L"]" << std::dec;
#endif
            if (SUCCEEDED(hr)) {
              DWRITE_TEXT_RANGE textRange;
...
#ifdef DEBUG_GPU_RESOURCE
            LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << pBackgroundColorBrush
                                     << L"]" << std::dec;
#endif
            if (FAILED(hr)) {
              SPDLOG_ERROR(L"CreateSolidColorBrush failed, hr = {:x}", hr);
...
void touchmind::control::DWriteEditControl::RenderHighlight(ID2D1RenderTarget *pRenderTarget) {
  ...
  if (m_caretVisible && acpStart != acpEnd && m_pHighlightBrush != nullptr) {
    ...
    UINT32 actualHitTextCount;
    m_pDWriteTextLayout->HitTestTextRange(acpStart, acpEnd - acpStart, x, y, nullptr, 0, &actualHitTextCount);
    if (actualHitTextCount > 0) {
      std::vector<DWRITE_HIT_TEST_METRICS> hitTestMetrics(actualHitTextCount);
      m_pDWriteTextLayout->HitTestTextRange(acpStart, acpEnd - acpStart, x, y, hitTestMetrics.data(),
                                            actualHitTextCount, &actualHitTextCount);
...
void touchmind::control::DWriteEditControl::OnRender(ID2D1RenderTarget *pRenderTarget) {
  ...
  FLOAT width, height;
  if (HasFocus()) {
    width = std::max(m_layoutRect.right - m_layoutRect.left, m_actualRect.right - m_actualRect.left);
    height = std::max(m_layoutRect.bottom - m_layoutRect.top, m_actualRect.bottom - m_actualRect.top);
  } else {
    width = m_actualRect.right - m_actualRect.left;
    height = m_actualRect.bottom - m_actualRect.top;
  }
  D2D1_RECT_F renderRect
      = D2D1::RectF(m_layoutRect.left, m_layoutRect.top, m_layoutRect.left + width, m_layoutRect.top + height);
...
void touchmind::control::DWriteEditControl::_CalculateCaretSizeAndPosition(D2D1_POINT_2F *caretPosInModel,
                                                                           DWRITE_HIT_TEST_METRICS *pHitTestMetrics) {
  ...
  m_pDWriteTextLayout->HitTestTextPosition(caretPosition, activeSelEnd == TS_AE_END, &(caretPosInModel->x),
                                           &(caretPosInModel->y), pHitTestMetrics);
...
void touchmind::control::DWriteEditControl::CreateCaret() {
  ...
  touchmind::util::CoordinateConversion::ConvertModelToWindowCoordinate(m_hWnd, m_pManager->GetScrollBarHelper(),
                                                                        caretPosInModel, &caretPosInWindow);
...
bool touchmind::control::DWriteEditControl::HitTest(FLOAT x, FLOAT y) {
  return m_layoutRect.left <= x && x <= m_layoutRect.right && m_layoutRect.top <= y && y <= m_layoutRect.bottom;
}
...
bool touchmind::control::DWriteEditControl::CalculateNewCaretPositionForUpDown(DWORD cursorDirection,
                                                                               OUT LONG *caretPosition,
  ...
  *caretPosition = (currentActiveSelEnd != TS_AE_END ? acpStart : acpEnd);
  ...
  GetLineFromPosition(lineMetrics.data(), static_cast<UINT32>(lineMetrics.size()), *caretPosition, &line,
                      &linePosition);
  ...
  m_pDWriteTextLayout->HitTestTextPosition(*caretPosition, (currentActiveSelEnd == TS_AE_END ? 1 : 0), &caretX, &caretY,
                                           &hitTestMetrics);

  // get a Y coordinate of the caret after moving lines
  m_pDWriteTextLayout->HitTestTextPosition(linePosition, false, &dummyX, &caretY, &hitTestMetrics);

  // We get the X and Y coordinate of the caret
  // get a new caret position(ACP)
  BOOL isInside, isTrailingHit;
  m_pDWriteTextLayout->HitTestPoint(caretX, caretY, &isTrailingHit, &isInside, &hitTestMetrics);
...
bool touchmind::control::DWriteEditControl::CalculateSelectionFromDragRect(D2D1_POINT_2F startPoint,
                                                                           D2D1_POINT_2F endPoint, OUT LONG *acpStart,
                                                                           OUT LONG *acpEnd,
                                                                           OUT TsActiveSelEnd *activeSelEnd) {
  BOOL isTrailingHit;
  BOOL isInside;
  ...
  m_pDWriteTextLayout->HitTestPoint((FLOAT)startPoint.x - x, (FLOAT)startPoint.y - y, &isTrailingHit, &isInside,
                                    &caretMetrics);
  if (!isInside) {
    return false;
  }
  *acpStart = caretMetrics.textPosition;
  ...
  m_pDWriteTextLayout->HitTestPoint((FLOAT)endPoint.x - x, (FLOAT)endPoint.y - y, &isTrailingHit, &isInside,
                                    &caretMetrics);
  if (!isInside) {
    return false;
  }
...
///<summary>get font attributes of the current selection. if no selection, return the font attributes of the caret
/// position</summary>
void touchmind::control::DWriteEditControl::GetFontInfoOfCurrentSelection(
    touchmind::text::FontAttributeCommand *pFontInfo) {
  if (m_pTextStoreACP->_IsLocked(TS_LF_READ)) {
    return;
  }
  m_pTextStoreACP->_LockDocument(TS_LF_READ);
...
void touchmind::control::DWriteEditControl::SetFontInfoToCurrentSelection(
    touchmind::text::FontAttributeCommand *pFontInfo) {
...
/// this method is invoked by OnMouseEvents、OnKeyDown_CursorUpDown、OnKeyDown_CursorLeftRight
...
void touchmind::control::DWriteEditControl::CopySinglePropertyRange(
    IDWriteTextLayout *oldLayout, UINT32 startPosForOld, IDWriteTextLayout *newLayout, UINT32 startPosForNew,
    UINT32 length, touchmind::text::FontAttributeCommand *pFontAttributeCommand) {
...
void touchmind::control::DWriteEditControl::CopyRangedProperties(
    IDWriteTextLayout *oldLayout, UINT32 startPos, UINT32 endPos,
...
void touchmind::control::DWriteEditControl::CopyGlobalProperties(IDWriteTextLayout *oldLayout,
                                                                 IDWriteTextLayout *newLayout) {
...
void touchmind::control::DWriteEditControl::NotifyTextReplace(LONG oldTextLength, LONG startPos,
                                                              LONG replacedTextLength, LONG lengthOfNewText) {
  HRESULT hr = S_OK;
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (startPos == 0) {
      ...
      CopyRangedProperties(oldLayout, replacedTextLength, oldTextLength, std::abs(lengthOfNewText - replacedTextLength),
                           newLayout, lengthOfNewText < replacedTextLength);
    } else {
      ...
      CopyRangedProperties(oldLayout, startPos + replacedTextLength, oldTextLength,
                           std::abs(lengthOfNewText - replacedTextLength), newLayout,
...
void touchmind::control::DWriteEditControl::NotifyTextInsert(LONG oldTextLength, LONG startPos,
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (startPos == 0) {
      ...
      CopyRangedProperties(oldLayout, 0, startPos, 0, newLayout);
      CopySinglePropertyRange(oldLayout, startPos - 1, newLayout, startPos, textToInsertLength,
                              &m_caretFontAttributeCommand);
...
void touchmind::control::DWriteEditControl::UpdateFontInfoAtCaret() {
  LONG currentPos
      = m_pTextStoreACP->GetActiveSelEnd() != TF_AE_END ? m_pTextStoreACP->GetACPStart() : m_pTextStoreACP->GetACPEnd();
  if (currentPos > 0) {
    --currentPos;
  }
...
HRESULT
touchmind::control::DWriteEditControl::GetFontAttributes(std::vector<touchmind::text::FontAttribute> &fontAttributes) {
  HRESULT hr = S_OK;
  ...
  while (pos < m_pTextStoreACP->GetTextLength()) {
    ...
    fontAttribute.length = std::min(static_cast<LONG>(m_pTextStoreACP->GetTextLength() - actualTextRange.startPosition),
...
HRESULT
touchmind::control::DWriteEditControl::SetFontAttributes(touchmind::Configuration *pConfiguration,
                                                         std::vector<touchmind::text::FontAttribute> &fontAttributes) {
  HRESULT hr = S_OK;
  for (auto &fa : fontAttributes) {
    DWRITE_TEXT_RANGE textRange;
    textRange.startPosition = fa.startPosition;
    textRange.length = fa.length;
    touchmind::text::FontAttributeCommand fontAttributeCommand;
    fontAttributeCommand.FromFontAttribute(pConfiguration, &fa);
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pForegroundBrush << L"]"
                             << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pBackgroundBrush << L"]"
                             << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pBorderLineBrush << L"]"
                             << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pForegroundBrushForEdit
                             << L"]" << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pBackgroundBrushForEdit
                             << L"]" << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pBorderLineBrushForEdit
                             << L"]" << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pHighlightBrush << L"]"
                             << std::dec;
#endif
    if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1SolidColorBrush = [" << std::hex << m_pCompositionUnderlineBrush
                             << L"]" << std::dec;
#endif
    if (FAILED(hr)) {
...
HRESULT touchmind::control::DWriteEditControl::CreateDeviceIndependentResources() {
  ...
  hr = pD2DFactory->CreateStrokeStyle(D2D1::StrokeStyleProperties(D2D1_CAP_STYLE_FLAT, D2D1_CAP_STYLE_FLAT,
                                      ...
                                      nullptr, 0, &m_pCompositionSolidUnderlineStrokeStyle);
#ifdef DEBUG_GPU_RESOURCE
  LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1StrokeStyle = [" << std::hex
  ...
  hr = pD2DFactory->CreateStrokeStyle(D2D1::StrokeStyleProperties(D2D1_CAP_STYLE_ROUND, D2D1_CAP_STYLE_ROUND,
                                      ...
                                      nullptr, 0, &m_pCompositionDotUnderlineStrokeStyle);
#ifdef DEBUG_GPU_RESOURCE
  LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1StrokeStyle = [" << std::hex
  ...
  hr = pD2DFactory->CreateStrokeStyle(D2D1::StrokeStyleProperties(D2D1_CAP_STYLE_FLAT, D2D1_CAP_STYLE_FLAT,
                                      ...
                                      nullptr, 0, &m_pCompositionDashUnderlineStrokeStyle);
#ifdef DEBUG_GPU_RESOURCE
  LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1StrokeStyle = [" << std::hex
  ...
  hr = pD2DFactory->CreateStrokeStyle(D2D1::StrokeStyleProperties(D2D1_CAP_STYLE_FLAT, D2D1_CAP_STYLE_FLAT,
                                                                  D2D1_CAP_STYLE_FLAT, D2D1_LINE_JOIN_MITER, 10.0f,
                                                                  D2D1_DASH_STYLE_DASH_DOT, 0.0f),
                                      nullptr, 0, &m_pCompositionSquiggleUnderlineStrokeStyle);
#ifdef DEBUG_GPU_RESOURCE
  LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1StrokeStyle = [" << std::hex
...
void touchmind::control::DWriteEditControl::AddCompositionRenderInfo(LONG startPos, LONG endPos,
...
void touchmind::control::DWriteEditControl::OnDebugDump(std::wofstream &os) {
  os << L"DWriteEditControl[layoutRect=(" << m_layoutRect.left << L"," << m_layoutRect.top << L")-("
     << m_layoutRect.right << L"," << m_layoutRect.bottom << L"), actualRect=(" << m_layoutRect.left << L","
     << m_layoutRect.top << L")-(" << m_layoutRect.right << L"," << m_layoutRect.bottom << L")]" << std::endl;
}

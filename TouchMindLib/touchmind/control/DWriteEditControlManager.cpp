...
void touchmind::control::DWriteEditControlManager::Initialize(HWND hWnd, touchmind::Configuration *pConfiguration,
...
HRESULT touchmind::control::DWriteEditControlManager::CreateEditControl(IN D2D1_RECT_F layoutRect,
                                                                        IN const std::wstring &text,
                                                                        OUT EDIT_CONTROL_INDEX *pEditControlIndex) {
  std::shared_ptr<control::DWriteEditControl> pEditControl
      = std::make_shared<DWriteEditControl>(this, layoutRect, m_defaultFontInfo);
...
std::shared_ptr<touchmind::control::DWriteEditControl>
touchmind::control::DWriteEditControlManager::GetEditControl(IN EDIT_CONTROL_INDEX index) {
...
touchmind::control::EDIT_CONTROL_INDEX touchmind::control::DWriteEditControlManager::GetEditControlIndex(
  ...
  auto result = std::find_if(m_dwEditControlMap.begin(), m_dwEditControlMap.end(),
                             [&](EditControlIndexToEditControlPtrPair e) { return e.second == pEditControl; });
...
void touchmind::control::DWriteEditControlManager::DestroyDWLightEditControl(IN EDIT_CONTROL_INDEX editControlIndex) {
  auto result = std::find_if(m_dwEditControlMap.begin(), m_dwEditControlMap.end(),
                             [&](EditControlIndexToEditControlPtrPair e) { return e.first == editControlIndex; });
...
void touchmind::control::DWriteEditControlManager::OnRender(ID2D1RenderTarget *pRenderTarget) {
  ...
  for (auto &kv : m_dwEditControlMap) {
    kv.second->OnRender(pRenderTarget);
  }
}

std::shared_ptr<touchmind::control::DWriteEditControl> touchmind::control::DWriteEditControlManager::HitText(FLOAT x,
                                                                                                             FLOAT y) {
  for (auto &kv : m_dwEditControlMap) {
    if (kv.second->HitTest(x, y)) {
      return kv.second;
    }
  }
  return nullptr;
...
HRESULT touchmind::control::DWriteEditControlManager::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam,
                                                              OUT bool *eaten) {
  *eaten = false;
...
std::shared_ptr<touchmind::control::DWriteEditControl>
touchmind::control::DWriteEditControlManager::GetFocusedEditControl() {
...
void touchmind::control::DWriteEditControlManager::GetFontInfoFromIPropertyStore(
    IN IPropertyStore *pPropertyStore, OUT touchmind::text::FontAttributeCommand *pFontInfo) {
  PROPVARIANT propvar;
  ...
  if (SUCCEEDED(pPropertyStore->GetValue(UI_PKEY_FontProperties_ForegroundColor, &propvar))) {
    ...
    pFontInfo->foregroundColor
        = D2D1::ColorF(GetRValue(colorRef) / 255.0f, GetGValue(colorRef) / 255.0f, GetBValue(colorRef) / 255.0f, 1.0f);
...
void touchmind::control::DWriteEditControlManager::GetIPropertyStoreFromFontInfo(
    IN touchmind::text::FontAttributeCommand *pFontInfo, OUT IPropertyStore *pPropertyStore) {
  PROPVARIANT propvar;
  PropVariantInit(&propvar);
  ...
  if (pFontInfo->foregroundColorStatus != UI_FONTPROPERTIES_NOTAVAILABLE) {
    ...
    COLORREF colorRef = RGB(pFontInfo->foregroundColor.r * 255, pFontInfo->foregroundColor.g * 255,
                            pFontInfo->foregroundColor.b * 255);
...
void touchmind::control::DWriteEditControlManager::AddEditControlChangeListener(
    IDWLightEditControlChangeListener *pListener) {
  m_editControlChangeListener.push_back(pListener);
}

void touchmind::control::DWriteEditControlManager::RemoveEditControlChangeListener(
    IDWLightEditControlChangeListener *pListener) {
  m_editControlChangeListener.erase(
      std::remove(m_editControlChangeListener.begin(), m_editControlChangeListener.end(), pListener),
      m_editControlChangeListener.end());
}

void touchmind::control::DWriteEditControlManager::_FireEditControlChanged(EDIT_CONTROL_INDEX editControlIndex) {
  for (auto &l : m_editControlChangeListener) {
    l->NotifyEditControlHasChanged(editControlIndex);
  }
}
...
void touchmind::control::DWriteEditControlManager::NotifySelectionHasChanged(
  ...
  if (editControlIndex >= 0) {
    for (auto &l : m_selectionChangeListener) {
      l->NotifySelectionHasChanged(editControlIndex);
    }
  }
}

void touchmind::control::DWriteEditControlManager::AddSelectionChangeListener(
    IDWLightEditControlSelectionChangeListener *pListener) {
  m_selectionChangeListener.push_back(pListener);
}

void touchmind::control::DWriteEditControlManager::RemoveSelectionChangeListener(
    IDWLightEditControlSelectionChangeListener *pListener) {
  m_selectionChangeListener.erase(
      std::remove(m_selectionChangeListener.begin(), m_selectionChangeListener.end(), pListener),

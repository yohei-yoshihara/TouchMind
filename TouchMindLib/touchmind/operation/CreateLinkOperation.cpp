...
void touchmind::operation::CreateLinkOperation::CreateDeviceDependentResources(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget)
{
    if (m_foregroundBrush == nullptr) {
        CHK_RES(m_foregroundBrush, pRenderTarget->CreateSolidColorBrush(
                    D2D1::ColorF(1.0f, 1.0f, 1.0f, 1.0f),
                    &m_foregroundBrush));
    }
    // start node message
    if (m_startNodeBodyBrush == nullptr) {
        CHK_RES(m_startNodeBodyBrush, pRenderTarget->CreateSolidColorBrush(
                    D2D1::ColorF(D2D1::ColorF::Navy),
                    &m_startNodeBodyBrush));
    }
    if (m_startNodeMessageTextFormat == nullptr) {
        CHK_RES(m_startNodeMessageTextFormat, pContext->GetDWriteFactory()->CreateTextFormat(
                    MUI::GetString(IDS_DEFAULT_FONT_NAME),
                    nullptr,
                    ...
                    14.0f,
                    L"",
                    &m_startNodeMessageTextFormat));
    }
    if (m_startNodeMessageTextLayout == nullptr) {
        CHK_RES(m_startNodeMessageTextLayout, pContext->GetDWriteFactory()->CreateTextLayout(
                    m_startNodeMessasgeText.c_str(),
                    static_cast<UINT32>(m_startNodeMessasgeText.size()),
                    m_startNodeMessageTextFormat,
                    MESSAGE_BOX_WIDTH,
                    MESSAGE_BOX_HEIGHT,
                    &m_startNodeMessageTextLayout));
    }
    // end node message
    if (m_endNodeBodyBrush == nullptr) {
        CHK_RES(m_endNodeBodyBrush, pRenderTarget->CreateSolidColorBrush(
                    D2D1::ColorF(D2D1::ColorF::Orange),
                    &m_endNodeBodyBrush));
    }
    if (m_endNodeMessageTextFormat == nullptr) {
        CHK_RES(m_endNodeMessageTextFormat, pContext->GetDWriteFactory()->CreateTextFormat(
                    MUI::GetString(IDS_DEFAULT_FONT_NAME),
                    nullptr,
                    DWRITE_FONT_WEIGHT_NORMAL,
                    DWRITE_FONT_STYLE_NORMAL,
                    ...
                    14.0f,
                    L"",
                    &m_endNodeMessageTextFormat));
    }
    if (m_endNodeMessageTextLayout == nullptr) {
        CHK_RES(m_endNodeMessageTextLayout, pContext->GetDWriteFactory()->CreateTextLayout(
                    m_endNodeMessasgeText.c_str(),
                    static_cast<UINT32>(m_endNodeMessasgeText.size()),
                    m_endNodeMessageTextFormat,
                    MESSAGE_BOX_WIDTH,
                    MESSAGE_BOX_HEIGHT,
                    &m_endNodeMessageTextLayout));
    }
    // start node number
    if (m_startNodeNumberMarkBrush == nullptr) {
        CHK_RES(m_startNodeNumberMarkBrush, pRenderTarget->CreateSolidColorBrush(
                    D2D1::ColorF(0.0f, 0.0f, 1.0f, 0.5f),
                    &m_startNodeNumberMarkBrush));
    }
    if (m_startNodeNumberTextFormat == nullptr) {
        CHK_RES(m_startNodeNumberTextFormat, pContext->GetDWriteFactory()->CreateTextFormat(
                    MUI::GetString(IDS_DEFAULT_FONT_NAME),
                    nullptr,
                    DWRITE_FONT_WEIGHT_NORMAL,
                    DWRITE_FONT_STYLE_NORMAL,
                    ...
                    16.0f,
                    L"",
                    &m_startNodeNumberTextFormat));
    }
    if (m_startNodeNumberTextLayout == nullptr) {
        CHK_RES(m_startNodeMessageTextLayout, pContext->GetDWriteFactory()->CreateTextLayout(
                    m_startNodeNumberText.c_str(),
                    static_cast<UINT32>(m_startNodeNumberText.size()),
                    m_startNodeNumberTextFormat,
                    FLT_MAX,
                    FLT_MAX,
                    &m_startNodeNumberTextLayout));
    }
}
...
void touchmind::operation::CreateLinkOperation::StartSelectFirstNode(touchmind::Context *pContext)
{
    ...
    m_messageTextStoryboard->RepeatBetweenKeyframes(UI_ANIMATION_KEYFRAME_STORYBOARD_START, keyframe, UI_ANIMATION_REPEAT_INDEFINITELY);
...
void touchmind::operation::CreateLinkOperation::Draw(
    touchmind::Context *pContext,
    ID2D1RenderTarget *pRenderTarget )
{
    ...
    if (m_messageTextVar != nullptr) {
        CComPtr<IDWriteTextLayout> pTextLayout =
            ...
            m_startNodeMessageTextLayout : m_endNodeMessageTextLayout;
        CComPtr<ID2D1SolidColorBrush> pBrush =
            (m_startNode == nullptr && m_endNode == nullptr) ?
            m_startNodeBodyBrush : m_endNodeBodyBrush;
        ...
        D2D1_RECT_F bodyRect = D2D1::RectF(
                                   x,
                                   y,
                                   x + metrics.width + 10.0f * 2,
                                   y + metrics.height + 10.0f * 2);
        double alpha;
    ...
    if (m_startNode != nullptr) {
        ...
        pRenderTarget->DrawTextLayout(D2D1::Point2F(cx - metrics.width / 2.0f, cy - metrics.height / 2.0f), m_startNodeNumberTextLayout, m_foregroundBrush);
    }
    ...
    if (m_startNodeStoryboard != nullptr) {
        ...
        if (status == UI_ANIMATION_STORYBOARD_PLAYING) {
            ...
            pRenderTarget->DrawEllipse(D2D1::Ellipse(m_startNodeCenterPoint, static_cast<FLOAT>(radius), static_cast<FLOAT>(radius)), m_startNodeBodyBrush);
        }
    }
    if (m_endNodeStoryboard != nullptr) {
        ...
        if (status == UI_ANIMATION_STORYBOARD_PLAYING) {
            ...
            pRenderTarget->DrawEllipse(D2D1::Ellipse(m_endNodeCenterPoint, static_cast<FLOAT>(radius), static_cast<FLOAT>(radius)), m_endNodeBodyBrush);
        }
    }
}

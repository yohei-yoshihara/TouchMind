  ...
  if (m_executeListeners.count(nCmdID) > 0 || m_updatePropertyListeners.count(nCmdID) > 0) {
    ...
    *ppCommandHandler = nullptr;
    ...
    case cmdFontControl: {
      ...
      auto pFontControlHandler
          = dynamic_cast<touchmind::ribbon::handler::FontControlCommandHandler *>(m_pFontControlCommandHandler);
 ...
 */
HRESULT touchmind::ribbon::RibbonCommandHandler::OnDestroyUICommand(UINT32 commandId, UI_COMMANDTYPE typeID,

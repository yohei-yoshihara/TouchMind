...
HRESULT touchmind::ribbon::handler::NodeBackgroundColorCommandHandler::_CreateUIImage(D2D1_COLOR_F color,
  ...
  plateFigure.DrawPlate(D2D1::RectF(0.0f, 0.0f, static_cast<FLOAT>(width), static_cast<FLOAT>(height) * 0.7f),
                        plateFigure.GetCornerRoundSize(), m_pContext->GetD2DFactory(), pRenderTarget);
...
HRESULT
touchmind::ribbon::handler::NodeBackgroundColorCommandHandler::CreateInstance(IUICommandHandler **ppCommandHandler) {
  HRESULT hr = S_OK;
  if (!ppCommandHandler) {
    hr = E_POINTER;
  } else {
...
IFACEMETHODIMP touchmind::ribbon::handler::NodeBackgroundColorCommandHandler::QueryInterface(REFIID riid,
                                                                                             void **ppInterface) {
...
IFACEMETHODIMP touchmind::ribbon::handler::NodeBackgroundColorCommandHandler::Execute(
  ...
  if (pKey && *pKey == UI_PKEY_ColorType) {
    ...
    m_pRibbonFramework->GetFramework()->InvalidateUICommand(cmdBackgroundColor, UI_INVALIDATIONS_PROPERTY,
                                                            &UI_PKEY_LargeImage);
  }
  return hr;
}

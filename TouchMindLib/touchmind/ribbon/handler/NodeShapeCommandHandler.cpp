...
const std::array<int, NUMBER_OF_NODE_SHAPES> touchmind::ribbon::handler::NodeShapeCommandHandler::s_IMAGE_ID = {
    IDB_NODESTYLE_ROUNDEDRECT, IDB_NODESTYLE_RECTANGLE,
};
...
IFACEMETHODIMP
touchmind::ribbon::handler::NodeShapeCommandHandler::Execute(UINT cmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY *pKey,
...
IFACEMETHODIMP touchmind::ribbon::handler::NodeShapeCommandHandler::UpdateProperty(
  ...
  if (key == UI_PKEY_Enabled) {
    ...
    m_pRibbonFramework->GetFramework()->InvalidateUICommand(cmdNodeShape, UI_INVALIDATIONS_PROPERTY,
    ...
    THROW_IF_FAILED(pPropvarCurrentValue->punkVal->QueryInterface(

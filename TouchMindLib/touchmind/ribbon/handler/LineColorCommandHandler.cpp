...
IFACEMETHODIMP touchmind::ribbon::handler::LineColorCommandHandler::Execute(
    UINT cmdID,
    ...
    IUISimplePropertySet* pCommandExecutionProperties)
{
    ...
    if (pKey && *pKey == UI_PKEY_ColorType) {
        ...
        m_pRibbonFramework->GetFramework()->InvalidateUICommand(cmdLineColor, UI_INVALIDATIONS_PROPERTY, &UI_PKEY_LargeImage);
    }
    return hr;
}
    ...
    PROPVARIANT* pPropvarNewValue)
{
    ...
    if (key == UI_PKEY_Enabled) {
        ...
        m_pRibbonFramework->GetFramework()->InvalidateUICommand(cmdLineColor, UI_INVALIDATIONS_PROPERTY, &UI_PKEY_ColorType);
...
HRESULT touchmind::ribbon::handler::LineColorCommandHandler::_CreateUIImage(D2D1_COLOR_F color, IUIImage **ppUIImage)
{
    ...
    if (m_pContext->GetWICImagingFactory() == nullptr ||
            m_pContext->GetD2DFactory() == nullptr ||
    ...
    CHK_RES(pGeometry, view::GeometryBuilder::CreateCurvePathGeometry(
                m_pContext->GetD2DFactory(),
                curvePoints,
                &pGeometry));

...
namespace touchmind {
  namespace ribbon {
    ...
    typedef std::function<HRESULT(UINT nCmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY *key,
                                  const PROPVARIANT *pPropvarValue, IUISimplePropertySet *pCommandExecutionProperties)>
        OnExecuteListener;

    typedef std::function<HRESULT(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT *pPropvarCurrentValue,
                                  PROPVARIANT *pPropvarNewValue)> OnUpdatePropertyListener;

    class RibbonCommandHandler : public IUIApplication, public IUICommandHandler {
    private:
      ULONG m_refCount;
      IUIRibbon *m_pRibbon;
    ...
    public:
      ...
      IFACEMETHODIMP Execute(UINT nCmdID, UI_EXECUTIONVERB verb, const PROPERTYKEY *key,
                             const PROPVARIANT *ppropvarValue,
                             IUISimplePropertySet *pCommandExecutionProperties) override;
      IFACEMETHODIMP UpdateProperty(UINT nCmdID, REFPROPERTYKEY key, const PROPVARIANT *ppropvarCurrentValue,

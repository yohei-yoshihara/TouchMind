...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::SelectionChanged(
  ...
  for (auto &l : m_selectionChangedListeners) {
    l(m_pRibbonFramework, oldSelectedItem, newSelectedItem);
  }
}
...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::Execute_LineColor(UI_EXECUTIONVERB verb,
                                                                             D2D1_COLOR_F color) {
  if (m_pSelectionManager == nullptr) {
    return;
  }
...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::Execute_LineEdgeStyle(UI_EXECUTIONVERB verb,
                                                                                 LINE_EDGE_STYLE style) {
  if (m_pSelectionManager == nullptr) {
    return;
  }
  auto selectable = m_pSelectionManager->GetSelected();
...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::Execute_NodeBackgroundColor(UI_EXECUTIONVERB verb,
                                                                                       D2D1_COLOR_F color) {
  if (m_pSelectionManager == nullptr) {
    return;
  }
...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::Execute_NodeShape(UI_EXECUTIONVERB verb,
...
void touchmind::ribbon::dispatch::RibbonRequestDispatcher::_FireInvalidateCallback() {
  for (auto &l : m_invalidateCallbacks) {
    l();
  }
}

...
/*static*/
const SHORT touchmind::Context::s_FacesIndexArray[] = {
    3, 1, 0,
    2, 1, 3
};
...
#ifdef DEBUG_GPU_RESOURCE
    LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10RasterizerState = [" << std::hex << m_pRasterizerState << L"]" << std::dec;
#endif
    if (SUCCEEDED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10InputLayout = [" << std::hex << m_pVertexLayout << L"]" << std::dec;
#endif
        if (SUCCEEDED(hr)) {
...
HRESULT touchmind::Context::_RecreateSizedResources(UINT nWidth, UINT nHeight)
{
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10Texture2D = [" << std::hex << m_pDepthStencil << L"]" << std::dec;
#endif
        if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10RenderTargetView = [" << std::hex << m_pBackBufferRenderTargetView << L"]" << std::dec;
#endif
        if (FAILED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        depthViewDesc.ViewDimension = (m_msaaQuality == 0 ? D3D10_DSV_DIMENSION_TEXTURE2D : D3D10_DSV_DIMENSION_TEXTURE2DMS);
        depthViewDesc.Texture2D.MipSlice = 0;

        SafeRelease(&m_pDepthStencilView);
        hr = m_pDevice->CreateDepthStencilView(m_pDepthStencil, &depthViewDesc, &m_pDepthStencilView);
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10DepthStencilView = [" << std::hex << m_pDepthStencilView << L"]" << std::dec;
#endif
        if (FAILED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10Texture2D = [" << std::hex << m_pD2DTexture2D << L"]" << std::dec;
#endif
        if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
            LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10ShaderResourceView = [" << std::hex << m_pD2DTexture2DResourceView << L"]" << std::dec;
#endif
            if (FAILED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = m_pD2DFactory->CreateDxgiSurfaceRenderTarget(
                     pDxgiSurface1,
                     &props,
                     &m_pD2DTexture2DRenderTarget
                 );
#ifdef DEBUG_GPU_RESOURCE
            LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1RenderTarget = [" << std::hex << m_pD2DTexture2DRenderTarget << L"]" << std::dec;
#endif
            if (FAILED(hr)) {
...
HRESULT touchmind::Context::_CreateD3DDevice(
...
HRESULT touchmind::Context::LoadResourceBitmap(
...
HRESULT touchmind::Context::LoadResourceShader(
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] IDWriteFactory = [" << std::hex << m_pDWriteFactory << L"]" << std::dec;
#endif
    }
...
HRESULT touchmind::Context::CreateD2DRenderTargetFromTexture2D(ID3D10Texture2D* pTexure2D, ID2D1RenderTarget** ppRenderTarget)
{
#ifdef TOUCHMIND_CONTEXT_DEBUG
    LOG_ENTER;
#endif
    ...
    if (SUCCEEDED(hr)) {
        hr = m_pD2DFactory->CreateDxgiSurfaceRenderTarget(
                 ...
                 ppRenderTarget);
#ifdef DEBUG_OUTPUT_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID2D1RenderTarget = [" << std::hex << *ppRenderTarget << L"]" << std::dec;
#endif
        if (FAILED(hr)) {
...
#ifdef DEBUG_GPU_RESOURCE
        LOG(SEVERITY_LEVEL_INFO) << L"[GPU RESOURCE] ID3D10Buffer = [" << std::hex << m_pFacesIndexBuffer << L"]" << std::dec;
#endif
    }
#ifdef TOUCHMIND_CONTEXT_DEBUG
    LOG_LEAVE;
...
HRESULT touchmind::Context::CreateD2DSharedBitmapFromTexture2D(ID2D1RenderTarget* pRenderTarget, ID3D10Texture2D* pTexure2D, ID2D1Bitmap** ppBitmap)
{
#ifdef TOUCHMIND_CONTEXT_DEBUG
    LOG_ENTER;
#endif
...
HRESULT touchmind::Context::_InitializeAnimation()
{
    ...
    if (SUCCEEDED(hr)) {
        hr = touchmind::animation::AnimationManagerEventHandler::CreateInstance(

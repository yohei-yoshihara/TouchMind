...
LRESULT CALLBACK touchmind::win::CanvasPanel::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_CREATE) {
    ...
    ::SetWindowLongPtrW(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pCanvasPanel));

    pCanvasPanel->OnCreate();
    result = 1;
  } else {
    touchmind::touch::ManipulationResult manipulationResult;
    auto *pCanvasPanel = reinterpret_cast<touchmind::win::CanvasPanel *>(
    ...
    if (pCanvasPanel) {
      ...
      if (eaten) {
        ...
        case WM_LBUTTONDBLCLK:
          manipulationResult
              = pCanvasPanel->m_pManipulationHelper->OnLeftMouseDoubleClick(hWnd, message, wParam, lParam);
          break;
...
void touchmind::win::CanvasPanel::Initialize(UINT ribbonHeight) {
  ...
  m_hwnd
      = CreateWindow(L"CanvasPanel", L"CanvasPanel", WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | WS_CLIPCHILDREN,
...
void touchmind::win::CanvasPanel::_InitializeAfterCreateWindow() {
  ...
  m_pEditControlManager->Initialize(m_hwnd, m_pTouchMindApp->GetConfiguration(), m_pTouchMindApp->GetTSFManager(),
                                    m_pContext->GetD2DFactory(), m_pContext->GetDWriteFactory());
...
HRESULT touchmind::win::CanvasPanel::CreateD2DResources(touchmind::Context *pContext, ID2D1Factory *pD2DFactory,
...
HRESULT touchmind::win::CanvasPanel::Render2D(touchmind::Context *pContext, ID2D1RenderTarget *pRenderTarget) {
  HRESULT hr = S_OK;
...
void touchmind::win::CanvasPanel::ResizeLayoutRect(const std::shared_ptr<touchmind::model::node::NodeModel> &node,
...
void touchmind::win::CanvasPanel::AfterNew(touchmind::model::MapModel *pMapModel) {
  ...
  m_pNodeViewManager->SynchronizeWithModel();
  m_pLinkViewManager->Resynchronize(m_pContext.get(), m_pContext->GetD2DRenderTarget());
  Invalidate();
}
...
void touchmind::win::CanvasPanel::OnCreateLink_End() {
  m_canvasState = CANVAS_STATE_NORMAL;
  m_pTouchMindApp->GetRibbonFramework()->GetFramework()->InvalidateUICommand(cmdCreateLink, UI_INVALIDATIONS_VALUE,
                                                                             nullptr);
}

// IAnimationStatusChangedListener (start)
HRESULT touchmind::win::CanvasPanel::OnAnimationStatusChanged(UI_ANIMATION_MANAGER_STATUS newStatus,

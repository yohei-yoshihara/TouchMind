    ...
    public:
      ...
      touchmind::VISITOR_RESULT operator()(const std::shared_ptr<touchmind::model::node::NodeModel> &node) {
        if (((!node->IsCollapsed() && !node->IsAncestorCollapsed())
            ...
            && (node->GetX() - m_margin) <= m_clientPoint.x
            && m_clientPoint.x <= (node->GetX() + node->GetWidth() + m_margin)
            && (node->GetY() - m_margin) <= m_clientPoint.y
            && m_clientPoint.y <= (node->GetY() + node->GetHeight() + m_margin)) {
          m_result = node;

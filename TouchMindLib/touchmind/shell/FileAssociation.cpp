...
HRESULT touchmind::shell::FileAssociation::RegSetString(HKEY hKey, std::wstring &subkey, std::wstring &value, std::wstring &data)
{
    HRESULT hr = S_OK;
    hr = HRESULT_FROM_WIN32(SHSetValue(hKey, subkey.c_str(), value.c_str(), REG_SZ, data.c_str(), static_cast<DWORD>((data.length() + 1) * sizeof(wchar_t))));
    return hr;
}
...
HRESULT touchmind::shell::FileAssociation::RegisterToHandleExt( const WCHAR *ext )
{
    ...
    if (SUCCEEDED(hr)) {
        hr = HRESULT_FROM_WIN32(RegCreateKeyEx(HKEY_CLASSES_ROOT, key.c_str(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, nullptr, &hkeyProgidList, nullptr));
    }

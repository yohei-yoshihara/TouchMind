...
public:
  __checkReturn static HRESULT CreateInstance(__in PWSTR wszDisplyName, __in PWSTR wszFullPath,
...
HRESULT touchmind::shell::MRUManager::PopulateRibbonRecentItems(__deref_out PROPVARIANT *pvarValue) {
  ...
  hr = CoCreateInstance(CLSID_ApplicationDocumentLists, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pDocumentLists));
  ...
  if (psa != nullptr) {
    ...
    while (iCurrentFile < static_cast<LONG>(iFileCount)) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        hr = pPropertiesObj->QueryInterface(__uuidof(IUnknown), reinterpret_cast<void **>(&pUnknown));
        if (SUCCEEDED(hr)) {
...
HRESULT touchmind::shell::MRUManager::GetSelectedItem(IN const PROPERTYKEY *key, IN const PROPVARIANT *ppropvarValue,

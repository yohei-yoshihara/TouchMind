...
{
    for (auto it = std::begin(m_fontMap); it != std::end(m_fontMap); ++it) {
        it->second->Release();
    }
    m_fontMap.clear();
...
STDMETHODIMP
touchmind::print::XpsDWriteTextRenderer::IsPixelSnappingDisabled(
...
STDMETHODIMP touchmind::print::XpsDWriteTextRenderer::GetCurrentTransform(
    void* /* clientDrawingContext */,
...
STDMETHODIMP touchmind::print::XpsDWriteTextRenderer::GetPixelsPerDip(
    void* /* clientDrawingContext */,
    FLOAT* pixelsPerDip
)
{
...
STDMETHODIMP
touchmind::print::XpsDWriteTextRenderer::DrawGlyphRun(
    void* clientDrawingContext,
    ...
    if (fontFaceType != DWRITE_FONT_FACE_TYPE_CFF &&
        ...
        if (SUCCEEDED(hr)) {
            for (UINT32 i = 0; i < glyphRun->glyphCount; ++i) {
                ...
                glyphIndexVector[i].horizontalOffset = (glyphRun->glyphOffsets != nullptr) ? FLOAT(glyphRun->glyphOffsets[i].advanceOffset * positionScale) : 0;
                glyphIndexVector[i].verticalOffset = (glyphRun->glyphOffsets != nullptr) ? FLOAT(glyphRun->glyphOffsets[i].ascenderOffset * positionScale) : 0;
            }
...
STDMETHODIMP touchmind::print::XpsDWriteTextRenderer::DrawUnderline(
    void* clientDrawingContext,
    FLOAT baselineOriginX,
    FLOAT baselineOriginY,
...
STDMETHODIMP
touchmind::print::XpsDWriteTextRenderer::DrawStrikethrough(
    void* clientDrawingContext,
    FLOAT baselineOriginX,
    FLOAT baselineOriginY,
...
STDMETHODIMP
touchmind::print::XpsDWriteTextRenderer::DrawInlineObject(
    void* /* clientDrawingContext */,
    FLOAT /* originX */,
    FLOAT /* originY */,
...
HRESULT touchmind::print::XpsDWriteTextRenderer::ClusterMapToMappingArray( IN const UINT16 *clusterMap, IN UINT32 mapLen, IN UINT32 glyphsArrayLen, OUT std::vector<XPS_GLYPH_MAPPING> glyphMapping )
{
    HRESULT hr = S_OK;
    UINT32 i = 0;
    UINT32 mapPos = 0;
...
HRESULT touchmind::print::XpsDWriteTextRenderer::LookupBrush(IN IUnknown *drawingEffect, OUT D2D1_COLOR_F *pColor, OUT std::wstring & brushKey)
{
    HRESULT hr = S_OK;
...
HRESULT touchmind::print::XpsDWriteTextRenderer::AddLinePath( const XPS_POINT *beginPoint, const XPS_POINT *endPoint, FLOAT thickness, IUnknown* clientDrawingEffect  )
{
    HRESULT hr = S_OK;
...
HRESULT touchmind::print::XpsDWriteTextRenderer::CreateInstance(
...
HRESULT touchmind::print::XpsDWriteTextRenderer::FindOrCreateFontResource(IDWriteFontFace* fontFace, IXpsOMFontResource** ppXpsFontResource)
{
    HRESULT hr = S_OK;
    ...
    *ppXpsFontResource = nullptr;
    ...
    if (*ppXpsFontResource) {
        ...
        if (SUCCEEDED(hr)) {
            hr = m_pXpsFactory->CreateFontResource(
                     fontResourceStream,
...
{
    for (auto it = std::begin(m_temporaryFileNames); it != std::end(m_temporaryFileNames); ++it) {
        DeleteFile(it->c_str());
    }

...
void assertTreeChildFirst(NodeModel::reference parent) {
  LARGE_INTEGER liBegin, liEnd, liFreq;
  QueryPerformanceFrequency(&liFreq);

  std::vector<touchmind::NODE_ID> v1;
  QueryPerformanceCounter(&liBegin);
  std::for_each(parent->Begin_ChildFirst(), parent->End_ChildFirst(),
                [&v1](NodeModel::reference e) { v1.push_back(e->GetId()); });
  ...
  QueryPerformanceCounter(&liBegin);
  parent->ApplyVisitor([&v2](std::shared_ptr<NodeModel> e) -> touchmind::VISITOR_RESULT {
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_rootOnly_001) {
  std::shared_ptr<NodeModel> node(std::make_shared<NodeModel>());
  node->SetId(1);

  auto begin_it = node->Begin_ChildFirst();
  auto end_it = node->End_ChildFirst();

  ASSERT_EQ(1, (*begin_it)->GetId());
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_rootOnly_002) {
  std::shared_ptr<NodeModel> node(std::make_shared<NodeModel>());
  node->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_rootOnly_003) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(1);

  assertTreeChildFirst(parent);
}

TEST(touchmind_model_node_ChildFirstNodeIterator, plus_oneChild_001) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_twoChild_001) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_grandchild_001) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(1);
  std::shared_ptr<NodeModel> child1(std::make_shared<NodeModel>());
  child1->SetId(2);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_two_children_have_two_children_001) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(0);
  std::shared_ptr<NodeModel> child1(std::make_shared<NodeModel>());
  child1->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, plus_two_children_have_two_children_002) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(0);
  std::shared_ptr<NodeModel> child1(std::make_shared<NodeModel>());
  child1->SetId(1);
...
TEST(touchmind_model_node_ChildtFirstNodeIterator, pattern_001) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(0);
  std::shared_ptr<NodeModel> child1(std::make_shared<NodeModel>());
  child1->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, pattern_002) {
  std::shared_ptr<NodeModel> parent(std::make_shared<NodeModel>());
  parent->SetId(0);
  std::shared_ptr<NodeModel> child1(std::make_shared<NodeModel>());
  child1->SetId(1);
...
TEST(touchmind_model_node_ChildFirstNodeIterator, random_001) {
  ...
  for (size_t loop = 0; loop < 300; ++loop) {
    ...
    /*
    wchar_t buf[4096];
    wsprintf(buf, L"c:\\temp\\ChildFirst_%d.txt", loop);
    std::wofstream of(buf);
    root->DumpAll(of);
    of.close();
    */
    assertTreeChildFirst(root);
  }
}
